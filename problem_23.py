...
def is_abundant(n: int):
    return sum(PrimeDecomposition(n).get_proper_divisors()) > n


if __name__ == "__main__":

    abundant = {i: is_abundant(i) for i in range(1,28123+1)}
    ...
    for i in range(1, 28123+1):
        ...
        # Check all pairs that sum to i
        for j in range(1, i):
            if abundant[j] and abundant[i-j]:
                can_be_written.add(i)
                break

    cannot_be_written = set(range(1, 28123+1)) - can_be_written
