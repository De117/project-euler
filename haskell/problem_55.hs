...
reverseAndAdd n = n + fromDigits (reverse (digits n))

palindrome :: Integral a => a -> Bool
palindrome n = digits n == reverse (digits n)

-- We do not count palindromes themselves as "zero-iteration" Lychrel numbers.
-- They must become palindromes *through* the reverse-and-add process.
lychrel :: Integral a => a -> Bool
lychrel = null . dropWhile (not . palindrome) . take 49 . tail . iterate reverseAndAdd
